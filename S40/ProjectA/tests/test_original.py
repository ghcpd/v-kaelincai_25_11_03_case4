...
from src.original_code import summarize_transactions
...
def test_summarize_transactions(case: Dict[str, Any]):
    ...
    if case.get("expect_exception"):
        with pytest.raises(eval(case["expect_exception"])):  # nosec: B307 - controlled values
...
@pytest.mark.parametrize("case", load_cases(), ids=lambda c: c["name"])
def test_summary_handles_repeated_execution(case: Dict[str, Any]):
    """Ensure that running the function twice yields the same output.

    The original implementation materialises input iterables; this regression
    guard ensures the behaviour stays deterministic.
    """
    ...
    assert first == second
    assert first["transaction_count"] == expected["transaction_count"]


def test_invalid_specification_errors():
