"""Test harness for the original implementation.

The runner loads structured scenarios, executes the baseline analytics
function, and records both functional and lightweight performance metrics.
"""
...
from src.original_code import analyze_transactions
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Execute transaction analytics test cases.")
    ...
    parser.add_argument("--log-file", type=Path, default=Path(__file__).resolve().parents[1] / "logs" / "log_original.txt")
    parser.add_argument("--perf-file", type=Path, default=Path(__file__).resolve().parents[1] / "performance" / "time_original.txt")
    parser.add_argument("--implementation-name", default="Project A - Original")
