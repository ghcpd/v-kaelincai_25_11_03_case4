"""Test harness for original (pre-refactor) implementation.
Generates metrics & logs for comparison.
"""
...
from original_code import compute_statistics  # type: ignore

TEST_DATA_PATH = ROOT / 'test_data.json'
LOG_DIR = PROJECT_ROOT / 'logs'
PERF_DIR = PROJECT_ROOT / 'performance'
LOG_FILE = LOG_DIR / 'log_original.txt'
TIME_FILE = PERF_DIR / 'time_original.txt'
...
def _generate_large(gen: Dict[str, Any]):
    ...
    return json.dumps(data)  # original code can parse JSON
...
def run_tests():
    ...
    for case in cases:
        ...
        # Determine pass/fail on expected keys only
        comparisons = {}
        pass_case = True
